...
class DatabaseManagerTest extends TestCase
{
    ...
    {
        $this->assertEquals($this->databasePath, $this->databaseManager->getDatabasePath());
    }
    ...
    public function testZipcodesTableIndexes(): void
    {
        ...
        foreach ($indexColumns as $indexName => $expectedColumn) {
            $stmt = $pdo->query("PRAGMA index_info($indexName)");
            if ($stmt === false) {
                throw new Exception('Unable to run query');
            }
            $info = $stmt->fetchAll(\PDO::FETCH_ASSOC);
            $this->assertEquals($expectedColumn, $info[0]['name'], "Index $indexName should be on column $expectedColumn");
        }
    }
}
